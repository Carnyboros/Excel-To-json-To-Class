...
{	
	...
	public bool ConvertExcelFileToJson(string filePath, string outputPath)
	{
		...
		for (int i = 0; i < excelData.Tables.Count; i++)
		{
			...
			else
			{
                ...
                WriteTextToFile(RemoveTrashCharacters(spreadSheetJson), outputPath + "/" + fileName + ".json");
	...
	{
		fSteam = File.Open(filePath, FileMode.Open, FileAccess.Read);
		...
		if (xlsRegex.IsMatch(filePath))
		{
			...
			excelReader = ExcelReaderFactory.CreateBinaryReader(fSteam);
		}
		else if (xlsxRegex.IsMatch(filePath))
		{
			// Reading from a OpenXml Excel file (2007 format; *.xlsx)
			excelReader = ExcelReaderFactory.CreateOpenXmlReader(fSteam);
		}
		else
		{
